...
FORBIDDEN={"seed","seed_phrase","mnemonic","private_key","secret_key","keypair",
"signature","signatures","signed_transaction","raw_transaction","serialized_transaction",
"transaction_bytes","instruction_bytes","access_token","refresh_token","service_account_key"}
...
def validate_treasury_registry(reg:Dict[str,Any], observed_slot:Optional[int]=None, max_slot_lag:int=5000)->Dict[str,Any]:
    ...
    for i,a in enumerate(accounts):
        ...
        seen.add(ta)
        role=a.get("role")
        if role not in {"distribution_reserve","burn_reserve","vesting_reserve","dao_treasury","operations"}: errors.append(f"{p}:unsupported_role")
        bal=a.get("observed_balance_raw")
        if not isinstance(bal,int) or bal<0: errors.append(f"{p}:invalid_balance")
        else:
            total += bal; roles[role]=roles.get(role,0)+bal
    ...
    return {"status":"PASS" if not errors and accounts else "FAIL_CLOSED","errors":sorted(errors),"account_count":len(accounts),"total_observed_raw":str(total),"role_balances_raw":{k:str(v) for k,v in sorted(roles.items())},"registry_sha256":sha256(reg)}
...
def distribution_preview(policy:Dict[str,Any], treasury_validation:Dict[str,Any], revenue_minor:int, eligible:List[Dict[str,Any]])->Dict[str,Any]:
    ...
    approved_pool_minor=revenue_minor*SHARE_BPS//10000; total_units=sum(max(0,int(x.get("activity_units",0))) for x in eligible); allocations=[]
    if not blockers and total_units>0:
        cap=int(dc["per_user_cap_raw"]); epoch_cap=int(dc["epoch_budget_cap_raw"]); budget=min(approved_pool_minor,epoch_cap)
        for x in eligible:
            raw=budget*x["activity_units"]//total_units; allocations.append({"subject_ref":x.get("subject_ref"),"amount_raw":min(raw,cap)})
    allocated=sum(x["amount_raw"] for x in allocations)
    return {"status":"FAIL_CLOSED" if blockers else "REVIEW_READY_NOT_EXECUTION_READY","blockers":sorted(set(blockers)),"approved_share_bps":SHARE_BPS,"approved_pool_minor":approved_pool_minor,"allocated_raw":allocated,"allocations":allocations,"execution_authorized":False,"transaction_bytes_created":False,"broadcast":False}
...
def approval_manifest(kind:str, treasury_policy:Dict[str,Any], evidence_sha256:Iterable[str])->Dict[str,Any]:
    ...
    out={"schema":"thf-tokenops-approval-intent/v1","kind":kind,"minimum_approvals":int(cls["minimum_approvals"]),"execution_model":cls["execution"],"evidence_sha256":ev,"execution_authorized":False,"signed":False,"submitted":False,"broadcast":False,"financial_effect":False}; out["manifest_sha256"]=sha256(out); return out

def incident_assessment(audit:Dict[str,Any])->Dict[str,Any]:
    checks={"network":audit.get("network")==NETWORK,"mint":audit.get("mint")==MINT,"decimals":int(audit.get("decimals",-1))==DECIMALS,"mint_authority_disabled":audit.get("mint_authority") is None,"freeze_authority_disabled":audit.get("freeze_authority") is None,"supply_floor":int(audit.get("supply_raw",0))>=SUPPLY_FLOOR_RAW,"supply_ceiling":int(audit.get("supply_raw",0))<=SUPPLY_CEILING_RAW}; failed=sorted(k for k,v in checks.items() if not v)
