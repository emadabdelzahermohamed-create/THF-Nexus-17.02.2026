...
rpc getTokenLargestAccounts "[\"$MINT\",{\"commitment\":\"confirmed\"}]" "$OUT/largest-accounts.json"
rpc getSignaturesForAddress "[\"$MINT\",{\"limit\":20,\"commitment\":\"confirmed\"}]" "$OUT/recent-signatures.json"
...
if [ "${#TOKEN_ACCOUNTS[@]}" -gt 0 ]; then
  ...
  rpc getMultipleAccounts "[$ACCOUNTS_JSON,{\"encoding\":\"jsonParsed\",\"commitment\":\"confirmed\"}]" "$OUT/largest-account-details.json"
else
  jq -n '{jsonrpc:"2.0",result:{value:[]},id:1}' > "$OUT/largest-account-details.json"
fi
...
supply=load('token-supply.json')['result']['value']
slot=load('rpc-slot.json')['result']
largest=load('largest-accounts.json')['result']['value']
details=load('largest-account-details.json')['result']['value']
sigs=load('recent-signatures.json')['result']
...
summary=[
 f"THF_TOKEN_READONLY_AUDIT=PASS",
 f"NETWORK=solana-mainnet-beta",
 f"MINT={mint}",
 f"RPC_SLOT={slot}",
 f"PROGRAM_ID={program}",
 f"DECIMALS={audit['decimals']}",
 f"SUPPLY_RAW={audit['supply_raw']}",
 f"SUPPLY_UI={audit['supply_ui']}",
 f"MINT_AUTHORITY={audit['mint_authority']}",
 f"FREEZE_AUTHORITY={audit['freeze_authority']}",
 f"RECENT_SIGNATURES={len(sigs)}",
 "TRANSACTION_CREATED=FALSE",
 "TRANSACTION_SIGNED=FALSE",
 "TRANSACTION_SUBMITTED=FALSE",
 "PRIVATE_KEY_USED=FALSE",
...
(
  cd "$OUT"
  sha256sum token-audit.json mint-account.json token-supply.json largest-accounts.json largest-account-details.json recent-signatures.json rpc-slot.json SUMMARY.txt > SHA256SUMS.txt
)
