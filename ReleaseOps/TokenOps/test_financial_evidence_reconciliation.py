...
class T(unittest.TestCase):
 ...
 def test_current_state_reconciliation_fail_closed(self):
  ...
  self.assertEqual(r["readiness"],"FAIL_CLOSED");self.assertEqual(r["distribution"]["theoretical_35_percent_budget_raw"],35000);self.assertIsNone(r["distribution"]["actionable_budget_raw"]);self.assertFalse(r["financial_effect"])
 ...
 def test_fully_populated_registry_still_never_authorizes(self):
  ...
  rr=validate_registry(reg);self.assertTrue(rr["authoritative_inventory_ready"]);p=copy.deepcopy(POLICY);p["distribution_controls"].update({"per_user_cap":1000,"epoch_budget_cap":5000,"distribution_reserve_account":token,"distribution_delivery_model":"claim"});p["signer_policy"]["production_policy_status"]="approved";r=reconcile(p,TREASURY,AUDIT,build_holder_evidence(AUDIT),rr,100000);self.assertEqual(r["distribution"]["actionable_budget_raw"],5000);self.assertFalse(r["distribution"]["allocation_authorized"]);self.assertFalse(r["burn"]["execution_authorized"])
