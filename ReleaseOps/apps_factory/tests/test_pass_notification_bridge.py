...
def setup(now=1000.0, package="com.topherofit.thf.pulse"):
    ...
    authority=Authority(sessions); adapter=Adapter(); h=http.NotificationHttpContract(registry)
    guard=bridge_mod.PassSessionDeliveryGuard(authority=authority,clock=lambda:now)
    d=dispatch.NotificationDispatcher(registry,{"test":adapter},session_guard=guard)
    ...
    return b,authority,registry,vault,adapter,d,p1,p2
...
def test_live_authority_rejects_stale_session_before_mutation_or_dispatch():
    b,a,r,v,adapter,d,p1,p2=setup(); created=b.register(principal=p1,body=body("provider-token-s1")); tid=created.body["token_id"]
    a.revoke_session("s1")
...
def test_background_dispatch_cannot_bypass_revoked_pass_session():
    b,a,r,v,adapter,d,p1,p2=setup(); created=b.register(principal=p1,body=body("provider-token-s1")); tid=created.body["token_id"]
    # Simulate Pass revocation where notification cleanup has not yet completed.
    a.revoke_session("s1")
...
def test_background_dispatch_rechecks_expiry_and_audience():
    b,a,r,v,adapter,d,p1,p2=setup(); created=b.register(principal=p1,body=body("provider-token-s1")); tid=created.body["token_id"]
    req=provider.DeliveryRequest(tid,"title.key","body.key","en","thf://pulse/home",False)
    a.sessions["s1"]=replace(a.sessions["s1"],expires_at=999.0)
    with pytest.raises(PermissionError,match="expired"):
        d.dispatch(subject="u1",session_id="s1",request=req)
    a.sessions["s1"]=replace(a.sessions["s1"],expires_at=1060.0,package_id="com.topherofit.thf.forge")
...
def test_parallel_sessions_cannot_mutate_or_dispatch_each_others_registration():
    b,a,r,v,adapter,d,p1,p2=setup(); one=b.register(principal=p1,body=body("provider-token-s1")); two=b.register(principal=p2,body=body("provider-token-s2"))
...
def test_logout_remains_fail_secure_when_notification_cleanup_fails():
    ...
    # Crucially, even though cleanup failed and the row remains active, background send fails closed.
    assert r.get(tid,subject="u1",session_id="s1").active
    req=provider.DeliveryRequest(tid,"title.key","body.key","en","thf://pulse/home",False)
    with pytest.raises(PermissionError,match="revoked"):
        d.dispatch(subject="u1",session_id="s1",request=req)
