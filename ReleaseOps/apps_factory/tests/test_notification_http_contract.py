...
def principal(subject="u1", authenticated=True):
    return http.SessionPrincipal(subject=subject,session_id="session-123",authenticated=authenticated)
...
def test_unknown_package_fails_closed_through_http_contract():
    ...
    with pytest.raises(PermissionError):
        c.register(principal=principal(),body={"package":"com.fake.app","provider":"fcm","provider_token":"provider-token-123456"})

def test_rotation_and_revoke_are_subject_scoped():
    c,r,_=contract()
    body={"package":"com.topherofit.thf.forge","provider":"fcm","provider_token":"old-token-123456"}
    first=c.register(principal=principal("u1"),body=body)
    tid=first.body["token_id"]
    with pytest.raises(PermissionError): c.revoke(principal=principal("u2"),body={"token_id":tid})
    rotated=c.rotate(principal=principal("u1"),body={"package":body["package"],"provider":"fcm","old_token_id":tid,"provider_token":"new-token-123456"})
    assert rotated.status==200 and rotated.body["generation"]==2
    assert not r.get(tid,subject="u1").active
    assert c.revoke(principal=principal("u1"),body={"token_id":rotated.body["token_id"]}).status==204

def test_logout_revokes_only_authenticated_subject_package():
    c,r,_=contract(); p="com.topherofit.thf.codex"
    a=c.register(principal=principal("u1"),body={"package":p,"provider":"fcm","provider_token":"token-user-one-123"})
    b=c.register(principal=principal("u2"),body={"package":p,"provider":"fcm","provider_token":"token-user-two-123"})
    out=c.logout(principal=principal("u1"),body={"package":p})
