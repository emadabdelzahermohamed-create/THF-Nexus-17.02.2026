...
EVENT_ID_TEMPLATES=(
 ...
 r"\$\(\s*['\"]#{id}['\"]\s*\)\s*\??\.\s*(?:onclick|addEventListener)",
...
def route_regex(route:str):
 esc=re.escape(route); esc=re.sub(r'\\\{[^}]+\\\}',r'[^/?#]+',esc)
...
def id_has_handler(fid:str,combined:str,event='click'):
 ...
 esc=re.escape(fid)
 if event=='click' and any(re.search(x.format(id=esc),combined,re.I) for x in EVENT_ID_TEMPLATES): return True
 generic=(
  rf"getElementById\(\s*['\"]{esc}['\"]\s*\)\s*\??\.\s*(?:on{event}|addEventListener\s*\(\s*['\"]{event})",
  rf"querySelector\(\s*['\"]#{esc}['\"]\s*\)\s*\??\.\s*(?:on{event}|addEventListener\s*\(\s*['\"]{event})",
  rf"\$\(\s*['\"]#{esc}['\"]\s*\)\s*\??\.\s*(?:on{event}|addEventListener\s*\(\s*['\"]{event})",
  rf"\b{esc}\s*\.\s*(?:on{event}|addEventListener\s*\(\s*['\"]{event})",
 )
 return any(re.search(x,combined,re.I) for x in generic)
...
def form_has_handler(fid:str,action:str,combined:str):
 ...
 esc=re.escape(fid)
 # Helper-bound form where the second argument is a concrete endpoint (wire(form,'/api/...')).
 if re.search(rf"\b\w+\s*\(\s*{esc}\s*,\s*['\"]/[^'\"]+['\"]",combined,re.I): return True
 return bool(re.search(rf"\b\w*(?:form|submit|bind)\w*\s*\(\s*{esc}\s*[,)]",combined,re.I))
...
def audit(root:Path):
 ...
 for p in text_files(root):
  ...
  backend.extend(BACKEND_ROUTE.findall(t)); api_refs.extend(FETCH.findall(t)); api_refs.extend(AXIOS.findall(t))
 backend=sorted(set(backend)); route_rx=[route_regex(r) for r in backend]
 orphan_buttons=[]; bad_anchors=[]; orphan_forms=[]
 for p,t in docs:
  rel=p.relative_to(root).as_posix(); handled_form_button_spans=[]
  for fm in FORM_BLOCK.finditer(t):
   attrs,body=fm.group(1),fm.group(2); fid=attr(ID,attrs); action=attr(ACTION,attrs); handled=form_has_handler(fid,action,combined)
   if not handled: orphan_forms.append({'file':rel,'id':fid or None,'action':action or None})
    ...
    for bm in BUTTON.finditer(body):
     ...
     if typ not in ('button','reset'): handled_form_button_spans.append((body_start+bm.start(),body_start+bm.end()))
  for bm in BUTTON.finditer(t):
   ...
   bid=attr(ID,attrs)
   if not id_has_handler(bid,combined,'click'): orphan_buttons.append({'file':rel,'id':bid or None,'attrs':' '.join(attrs.split())[:240]})
 ...
 for u in sorted(set(api_refs)):
  ...
  if not mapped and lp.endswith('/'): mapped=any(r.startswith(lp+'{') for r in backend)
  if not mapped and not re.search(r'\.(?:js|css|png|jpg|jpeg|webp|svg|ico|json)(?:[?#].*)?$',lp,re.I): unmapped.append(u)
 return {'schema':3,'root':str(root),'html_documents':len(docs),'backend_routes':backend,'ui_api_references':sorted(set(api_refs)),
