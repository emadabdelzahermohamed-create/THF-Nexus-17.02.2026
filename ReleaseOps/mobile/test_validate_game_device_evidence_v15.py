...
T=importlib.util.spec_from_file_location('t14',HERE/'test_validate_game_device_evidence_v14.py'); T14=importlib.util.module_from_spec(T); sys.modules[T.name]=T14; T.loader.exec_module(T14)
V3=M.V3
...
class V15(unittest.TestCase):
 ...
 def test_pid_must_match_resumed_pid(self):
  with tempfile.TemporaryDirectory() as td:
   root=Path(td); d=evidence('rift',self.rsha,root); row=d['process_provenance']['combat']; p=root/row['evidence_ref']; current=M.one(p.read_text(),'THF_RESUMED_PID'); mutate(d,root,'combat','THF_RESUMED_PID='+current,'THF_RESUMED_PID=9999'); self.assertTrue(any('resumed PID must equal' in x for x in M.validate_bundle(self.reg,self.rsha,d,root)))
