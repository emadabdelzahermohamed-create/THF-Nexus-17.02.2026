...
- [ ] Improve exercise-to-animation mapping, transitions, foot contact, IK hooks and camera framing. Production preserves a dominant Stage16A viewport, depth/lighting composition, model/target-aware framing, smooth interpolation, rig-attached muscle visualization, hysteresis-stabilized near-ground foot-bone visualization, and a time + velocity-qualified planted-contact visual anchor that requires a short stable dwell while the foot is near ground and sufficiently still and releases on lift or fast planar/vertical travel. It also retains bounded pelvis/hips-driven automatic camera follow, explicit replay fade-out/reset/fade-in blending, tightened athlete shadow frustum, dedicated sculpt/key spotlight and pelvis-reactive soft grounding shadow. Mobile motion controls use a compact overlay to reduce avatar occlusion. Exercise reference video is a compact in-app PiP so Stage16A remains visually dominant. Floor/contact visualization remains explicitly non-IK/non-pressure. Exercise detail labels name-matched/unmapped motion uncertified/reference-only. True foot-lock/IK and biomechanical certification remain open.
...
- [x] Latest Stage16A visual increment: snapshot `1789989736929` reached `ready`; QA timestamp `1789989759014` produced Web/mobile screenshots with 0 frontend, 0 backend and 0 network errors. The active session keeps canonical MPFB/MakeHuman Stage16A dominant while exercise reference video is a compact in-app PiP; mobile positioning uses logical inline direction for Arabic RTL / English LTR, and obsolete synthetic `demo-loop` humanoid CSS is removed. Prior contact-dwell, velocity qualification, grounding, replay blending, lighting and motion-aware camera improvements remain preserved. This is runtime/visual-increment evidence only; authenticated screenshot-reference acceptance remains open.
...
- Latest P0 Web visual increment: AppDeploy snapshot `1789989736929`; QA timestamp `1789989759014`; Web/mobile screenshots reported 0 frontend/backend/network errors.
- Latest visual evidence checkpoint: `22ba1ecbd63a02227662c07bb2c9e37ba42527d6` (`WEB_STAGE16A_VIDEO_PIP_DEPLOY_20260921.md`).
...
- Latest production visual evidence is snapshot `1789989736929` / QA `1789989759014`; dominant Stage16A + in-app video PiP + RTL/LTR logical positioning + removal of obsolete synthetic demo-loop CSS, together with prior contact-dwell, velocity qualification, grounding, replay blending, lighting and motion-aware camera improvements, are production evidence, not biomechanical certification or final reference acceptance.
